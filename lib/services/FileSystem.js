...
export class FileSystem {
  ...
  _ensureTestDirectories() {
    try {
      // Import Node's native fs for the existsSync check to avoid any module issues
      const fsNative = require("fs");

      // Synchronously create directories to ensure they exist before tests run
      if (!fsNative.existsSync(this.homeDir)) {
        try {
      ...
      if (!fsNative.existsSync(this.gitConfigDirPath)) {
        try {
      ...
      if (!fsNative.existsSync(this.gitConfigPath)) {
        try {
      ...
      if (!fsNative.existsSync(this.configFilePath)) {
        try {
