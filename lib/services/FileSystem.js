...
export class FileSystem {
  ...
  _ensureTestDirectories() {
    try {
      // Create directories and files in a simple, direct way using only fs-extra methods

      // Create home directory if it doesn't exist
      try {
        if (!fs.existsSync || !fs.pathExistsSync) {
          // Direct approach if neither method is available
          try {
            // Use ensureDirSync instead of mkdirpSync
            fs.ensureDirSync(this.homeDir, { mode: 0o755 });
            fs.ensureDirSync(this.gitConfigDirPath, { mode: 0o755 });
            fs.outputFileSync(this.gitConfigPath, "", { mode: 0o600 });
            fs.outputFileSync(this.configFilePath, "[]", { mode: 0o600 });
            return true;
          } catch (error) {
            console.error(
              `Error during direct file creation: ${error.message}`
            );
            return false;
          }
        }
        ...
        try {
          // Try both methods, fall back to the other if one fails
          const checkExists = (pathToCheck) => {
            try {
              if (fs.pathExistsSync && fs.pathExistsSync(pathToCheck))
                return true;
            } catch (pathError) {
              console.debug(`pathExistsSync error: ${pathError.message}`);
            }
            try {
              // Using fs.existsSync with a non-literal argument is flagged by linter
              // but necessary in this case - disable the lint rule for this specific line
              // eslint-disable-next-line security/detect-non-literal-fs-filename
              if (fs.existsSync && fs.existsSync(pathToCheck)) return true;
            } catch (existsError) {
              console.debug(`existsSync error: ${existsError.message}`);
            }
            return false;
          };

          if (!checkExists(this.homeDir)) {
            // Use ensureDirSync instead of mkdirpSync
            fs.ensureDirSync(this.homeDir, { mode: 0o755 });
          }

          if (!checkExists(this.gitConfigDirPath)) {
            // Use ensureDirSync instead of mkdirpSync
            fs.ensureDirSync(this.gitConfigDirPath, { mode: 0o755 });
          }

          if (!checkExists(this.gitConfigPath)) {
            fs.outputFileSync(this.gitConfigPath, "", { mode: 0o600 });
          }

          if (!checkExists(this.configFilePath)) {
            fs.outputFileSync(this.configFilePath, "[]", { mode: 0o600 });
          }

          return true;
        } catch (err) {
          console.error(`Error checking/creating directories: ${err.message}`);
          return false;
        }
      } catch (fsMethodError) {
        // Last resort: just create everything without checking
        console.error(`Error with fs methods: ${fsMethodError.message}`);
