  ...
  _ensureTestDirectories() {
    try {
      ...
      if (!fs.existsSync(this.gitConfigDirPath)) {
        fs.mkdirSync(this.gitConfigDirPath, { recursive: true });
      }
      // Touch files to ensure they exist (with empty content)
      if (!fs.existsSync(this.gitConfigPath)) {
        fs.writeFileSync(this.gitConfigPath, '', { mode: 0o600 });
      }
      if (!fs.existsSync(this.configFilePath)) {
        fs.writeFileSync(this.configFilePath, '[]', { mode: 0o600 });
      }
    } catch (error) {
      // Just log in constructor; don't throw
