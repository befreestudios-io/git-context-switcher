  ...
  _ensureTestDirectories() {
    try {
      ...
      if (!fs.existsSync(this.homeDir)) {
        try {
          fs.mkdirSync(this.homeDir, { recursive: true, mode: 0o755 });
        } catch (dirError) {
      ...
      if (!fs.existsSync(this.gitConfigDirPath)) {
        try {
          fs.mkdirSync(this.gitConfigDirPath, { recursive: true, mode: 0o755 });
        } catch (dirError) {
          if (dirError.code === "EACCES") {
            console.error(
      ...
      if (!fs.existsSync(this.gitConfigPath)) {
        try {
          ...
          const fd = fs.openSync(this.gitConfigPath, "wx", 0o600);
          fs.writeSync(fd, "");
          fs.closeSync(fd);
        } catch (fileError) {
      ...
      if (!fs.existsSync(this.configFilePath)) {
        try {
          // Use exclusive file flag to prevent race conditions
          const fd = fs.openSync(this.configFilePath, "wx", 0o600);
          fs.writeSync(fd, "[]");
          fs.closeSync(fd);
        } catch (fileError) {
          if (fileError.code === "EACCES") {
