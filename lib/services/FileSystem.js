    ...
    try {
      ...
      try {
        ...
        const createDirIfNeeded = (dirPath) => {
          try {
            // Try basic fs operations first
            if (fs.statSync && fs.mkdirSync) {
              try {
              ...
              // If stat/mkdir aren't available, try other fs-extra methods
              if (typeof fs.ensureDirSync === "function") {
                ...
                // Last resort - try to use outputFileSync to create parent directories
                fs.outputFileSync(path.join(dirPath, ".gitkeep"), "", {
                  mode: 0o644,
                });
              }
            }
        ...
        const createFileIfNeeded = (filePath, content) => {
          try {
            ...
            if (!fileExists) {
              ...
              // Try various file writing methods
              if (typeof fs.outputFileSync === "function") {
