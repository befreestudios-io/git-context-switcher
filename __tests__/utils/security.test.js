...
describe("Security Utils", () => {
  ...
  describe("validateFilePath", () => {
    test("should reject paths with directory traversal attempts", () => {
      ...
      paths.forEach((path) => {
        expect(() => security.validateFilePath(path)).toThrow(
  ...
  describe("sanitizeInput", () => {
    test("should remove potential dangerous characters", () => {
      const inputs = [
        ...
        {
          input: 'Path with <script>alert("XSS")</script>',
          expected: "Path with alertXSS",
        },
        { input: "Command; rm -rf /", expected: "Command rm -rf " },
        {
          input: "Input with && dangerous command",
          expected: "Input with  dangerous command",
        },
  ...
  describe("validateUrlPattern", () => {
    test("should allow valid URL patterns", () => {
      const validPatterns = [
        ...
        "github.com/*.git",
        "*.github.io",
        "git.company-name.com/team/*",
        "github.com/user/repo?branch=main",
      ];

      validPatterns.forEach((pattern) => {
        expect(security.validateUrlPattern(pattern)).toBe(true);
      });
    });

    test("should reject URL patterns with dangerous characters", () => {
      const invalidPatterns = [
        "github.com/user/repo<script>",
        "github.com/org/`rm -rf /`",
        "gitlab.com/user/$(dangerous)",
        "github.com/user/'dangerous'",
        "github.com/user/{dangerous}",
        "github.com/user/[dangerous]",
      ];

      invalidPatterns.forEach((pattern) => {
        expect(security.validateUrlPattern(pattern)).toBe(false);
      });
    });

    test("should handle edge cases", () => {
      expect(security.validateUrlPattern("")).toBe(false);
      expect(security.validateUrlPattern("   ")).toBe(false);
      expect(security.validateUrlPattern(null)).toBe(false);
      expect(security.validateUrlPattern(undefined)).toBe(false);
    });
  });
});
